...
class FolderSync:
    ...
    # Log every change to the log file
    ...
    # Starts the thread
    def start_sync(self):
        sync_thread = threading.Thread(target=self.sync_folders)
        sync_thread.daemon = True  # This allows the thread to exit when the main program does
        sync_thread.start()
        return sync_thread
